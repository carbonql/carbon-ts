...
const container = core.v1.container,
      deployment = apps.v1beta2.deployment,
...
const frontendDepl =
  ...
  |> container.deploy(3)
const frontendSvc = frontendDepl |> deployment.exposeWithLoadBalancer(80);
...
const redisMasterDepl =
  ...
  |> container.deploy();
const redisMasterSvc = redisMasterDepl |> deployment.exposeToCluster(redisPort);

const redisAgentDepl =
  container.make("redis-slave", "gcr.io/google_samples/gb-redisslave:v1", redisPort)
  |> merge(addResourcesAndDiscovery)
  |> container.deploy(2);
const redisAgentSvc = redisAgentDepl |> deployment.exposeToCluster(redisPort);
